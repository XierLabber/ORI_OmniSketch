 ...
 * @brief Implementation of CHDeltoid
 *
...
class CHDeltoid : public SketchBase<key_len, T> {
private:
  ...
  CHDeltoid(const CHDeltoid &) = delete;
  CHDeltoid(CHDeltoid &&) = delete;
...
public:
   ...
   *
   */
  CHDeltoid(int32_t num_hash, int32_t num_group, double cnt_no_ratio,
...
size_t CHDeltoid<key_len, no_layer, T, hash_t>::get_idx1_(size_t i, size_t j, size_t k) const{
    return i * num_group_ * (nbits_ + 1) + j * (nbits_ + 1) + k;
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
size_t CHDeltoid<key_len, no_layer, T, hash_t>::get_idx0_(size_t i, size_t j, size_t k) const{
    return i * num_group_ * (nbits_) + j * (nbits_) + k;
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
CHDeltoid<key_len, no_layer, T, hash_t>::CHDeltoid(
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void CHDeltoid<key_len, no_layer, T, hash_t>::update(const FlowKey<key_len> &flowkey,
                                         T val) {
#ifdef MY_DEBUG
  if(flow_cnt % 10000 == 0)
  {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
T CHDeltoid<key_len, no_layer, T, hash_t>::query(const FlowKey<key_len> &flowkey) const {
...
CHDeltoid<key_len, no_layer, T, hash_t>::getHeavyHitter(double threshold) const {
  T thresh = threshold;
  double val1 = 0;
  double val0 = 0;
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
size_t CHDeltoid<key_len, no_layer, T, hash_t>::size() const {
  ch1_->print_rate("CH1");
  ch0_->print_rate("CH0");
  return sizeof(CHDeltoid<key_len, no_layer, T, hash_t>) +
         num_hash_ * sizeof(hash_t) +
         ch1_->size() + ch0_->size();
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void CHDeltoid<key_len, no_layer, T, hash_t>::clear() {
  sum_ = 0;
  ch1_->clear();
  ch0_->clear();
}
