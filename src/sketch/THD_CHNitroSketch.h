/**
 * @file THD_CHNitroSketch
 * @author XierLabber
 * @brief Implementation of THD_CHNitroSketch
 *
...
class THD_CHNitroSketch : public SketchBase<key_len, T> {
public:
  /**
   * @brief Construct by specifying depth and width
   *
   */
  THD_CHNitroSketch(int depth, int width, 
                double cnt_no_ratio,
...
double THD_CHNitroSketch<key_len, no_layer, T, hash_t>::update_probs[8] = {
    1.0, 1.0 / 2, 1.0 / 4, 1.0 / 8, 1.0 / 16, 1.0 / 32, 1.0 / 64, 1.0 / 128};

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
size_t THD_CHNitroSketch<key_len, no_layer, T, hash_t>::get_idx(size_t i, size_t j) const{
    return i * width_ + j;
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
T THD_CHNitroSketch<key_len, no_layer, T, hash_t>::CHNS_getCnt(size_t idx) const{
    return ch->getCnt(idx);
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
THD_CHNitroSketch<key_len, no_layer, T, hash_t>::THD_CHNitroSketch(
    int depth, int width, 
    double cnt_no_ratio,
    const std::vector<size_t> &width_cnt, 
    const std::vector<size_t> &no_hash)
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void THD_CHNitroSketch<key_len, no_layer, T, hash_t>::alwaysLineRateUpdate(
    const FlowKey<key_len> &flowkey, T value) {
  __do_update(flowkey, value, update_prob_);
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void THD_CHNitroSketch<key_len, no_layer, T, hash_t>::alwaysCorrectUpdate(
    const FlowKey<key_len> &flowkey, T value) {
  if (isLineRateUpdate()) {
    __do_update(flowkey, value, update_prob_);
  } else {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
std::size_t THD_CHNitroSketch<key_len, no_layer, T, hash_t>::size() const{
  ch->print_rate("");
  printf("\n");
  return sizeof(THD_CHNitroSketch<key_len, no_layer, T, hash_t>) 
        + depth_ * 2 * sizeof(hash_t) 
        + ch->size();
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void THD_CHNitroSketch<key_len, no_layer, T, hash_t>::clear() {
  ch->clear();
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void THD_CHNitroSketch<key_len, no_layer, T, hash_t>::__do_update(const FlowKey<key_len> &flowkey,
                                         T value, double prob) {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
T THD_CHNitroSketch<key_len, no_layer, T, hash_t>::query(const FlowKey<key_len> &flowkey) const{
  T median;
  T values[depth_];
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void THD_CHNitroSketch<key_len, no_layer, T, hash_t>::getNextUpdate(double prob) {
  int sample = 1;
  if (prob < 1.0) {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
bool THD_CHNitroSketch<key_len, no_layer, T, hash_t>::isLineRateUpdate() {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void THD_CHNitroSketch<key_len, no_layer, T, hash_t>::adjustUpdateProb(double traffic_rate) {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void THD_CHNitroSketch<key_len, no_layer, T, hash_t>::update(
    const FlowKey<key_len> &flowkey, T value) {
  __do_update(flowkey, value, update_prob_);
}
