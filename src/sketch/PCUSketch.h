...
class PCUSketch : public SketchBase<key_len, T> {
private:
  ...
  PCUSketch(const PCUSketch &) = delete;
  PCUSketch(PCUSketch &&) = delete;
...
public:
   ...
   *
   */
  PCUSketch(int32_t word_num_, int32_t hash_num, int32_t lg_used_bits_, int32_t pyramid_depth);
  /**
...
PCUSketch<key_len, T, hash_t>::PCUSketch(int32_t word_num_, int32_t hash_num, int32_t lg_used_bits_, int32_t pyramid_depth_)
...
template <int32_t key_len, typename T, typename hash_t>
PCUSketch<key_len, T, hash_t>::~PCUSketch() {
  delete hash_fns;
  for(int i = 0; i < d; i++){
    delete[] counter[i];
  }
...
template <int32_t key_len, typename T, typename hash_t>
void PCUSketch<key_len, T, hash_t>::update(const FlowKey<key_len> &flowkey,
                                          T val) {
  T min_value = 1 << 30;
  ...
  if(min_value != MY_MASK){
    for(int i = 0; i < d; i++){
        value[i] = (counter[0][my_word_index] >> (counter_offset[i] << lg_used_bits)) & MY_MASK;
        counter[0][my_word_index] += (value[i] == min_value)? ((uint64_t)val << (counter_offset[i] << lg_used_bits)) : 0;
    }
    return;
  }
...
template <int32_t key_len, typename T, typename hash_t>
T PCUSketch<key_len, T, hash_t>::query(const FlowKey<key_len> &flowkey) const {
  T min_value = 1 << 30;
...
template <int32_t key_len, typename T, typename hash_t>
size_t PCUSketch<key_len, T, hash_t>::size() const {
  return sizeof(*this)                // instance
         + sizeof(uint64_t) * total_counter_num; // counter
}

template <int32_t key_len, typename T, typename hash_t>
void PCUSketch<key_len, T, hash_t>::clear() {
  for(int i = 0; i < pyramid_depth; i++){
    int cur_length = (word_num + (1 << i) - 1) >> i;
...
template <int32_t key_len, typename T, typename hash_t>
void PCUSketch<key_len, T, hash_t>::carry(int32_t index, T val){
  int left_or_right;	
  ...
  throw std::overflow_error(
            "Counter overflow at the last layer in PCU, overflow by " +
            std::to_string(val) + ".");
}

template <int32_t key_len, typename T, typename hash_t>
T PCUSketch<key_len, T, hash_t>::get_value(int32_t index) const
{
	int left_or_right;	
    ...
    throw std::overflow_error(
            "Query overflow at the last layer in PCU.");
}
