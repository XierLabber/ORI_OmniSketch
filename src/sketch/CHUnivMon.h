 ...
 * @brief Implementation of CHUnivMon
 *
...
class CHUnivMon : public SketchBase<key_len, T> {
private:
  int32_t depth;
  ...
  CHUnivMon(const CHUnivMon &) = delete;
  CHUnivMon(CHUnivMon &&) = delete;
...
public:
   ...
   *
   */
  CHUnivMon(int32_t depth_, int32_t width_, int32_t log_n, double cnt_no_ratio,
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
T CHUnivMon<key_len, no_layer, T, hash_t>::querySketch(int32_t sketch_idx, const FlowKey<key_len> &flowkey) const{
  T values[depth];
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void CHUnivMon<key_len, no_layer, T, hash_t>::updateSketch(int32_t sketch_idx, const FlowKey<key_len> &flowkey, T val){
  for(int i = 0; i < depth; i++){
    int32_t idx = CS_hash_fns[sketch_idx][i](flowkey) % width[sketch_idx];
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
int32_t CHUnivMon<key_len, no_layer, T, hash_t>::getCHIdx(int32_t sketch_id, int32_t r, int32_t c) const{
  return (width_idx[sketch_id] + c) + r * total_length;
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
int32_t CHUnivMon<key_len, no_layer, T, hash_t>::getGlobalHash(const FlowKey<key_len> &flowkey, int32_t layer) const{
  if(layer == 0){
    return 1;
  }
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
CHUnivMon<key_len, no_layer, T, hash_t>::CHUnivMon(int32_t depth_, int32_t width_,
                                     int32_t log_n, double cnt_no_ratio,
                                     const std::vector<size_t> &width_cnt,
                                     const std::vector<size_t> &no_hash)
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
CHUnivMon<key_len, no_layer, T, hash_t>::~CHUnivMon() {
  if (global_hash_fns)
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void CHUnivMon<key_len, no_layer, T, hash_t>::update(const FlowKey<key_len> &flowkey,
                                         T val) {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
T CHUnivMon<key_len, no_layer, T, hash_t>::query(const FlowKey<key_len> &flowkey) const {
  int level;
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
size_t CHUnivMon<key_len, no_layer, T, hash_t>::size() const {
  ch->print_rate("UnivMon CH");
  return sizeof(*this) + 
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void CHUnivMon<key_len, no_layer, T, hash_t>::clear() {
