/**
 * @file CHElasticSketch.h
 * @author XierLabber (you@domain.com)
 * @brief Implementation of CH Elastic Sketch
 *
...
class CHElasticSketch : public SketchBase<key_len, T> {
private:
  struct Entry {
  ...
  hash_t hash_h_;
  // light part
  CHCMSketch<key_len, no_layer, T, hash_t> cm_;

public:
  CHElasticSketch(int32_t num_buckets, int32_t num_per_bucket, int32_t l_depth,
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
CHElasticSketch<key_len, no_layer, T, hash_t>::CHElasticSketch(int32_t num_buckets,
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
size_t CHElasticSketch<key_len, no_layer, T, hash_t>::size() const {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void CHElasticSketch<key_len, no_layer, T, hash_t>::clear() {
  ch->clear();
  cm_.clear();
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
CHElasticSketch<key_len, no_layer, T, hash_t>::~CHElasticSketch() {
  delete[] buckets_[0];
  delete[] buckets_;
  delete[] ch;
}
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
int CHElasticSketch<key_len, no_layer, T, hash_t>::heavypartInsert(
  ...
  T min_counter_val = ch->getEstCnt(getCHIdx(index, 0));
  for (int32_t i = 0; i < num_per_bucket_ - 1; ++i) {
    int32_t chIdx = getCHIdx(index, i);
  ...
  int32_t chIdx = getCHIdx(index, num_per_bucket_ - 1);
  ...
  if (!JUDGE_IF_SWAP(min_counter_val, guard_val)) {
    ...
    int32_t minCHIdx = getCHIdx(index, min_counter);
    swap_key = buckets_[index][min_counter].flowkey_;
    swap_val = ch->getEstCnt(minCHIdx);
    ch->resetCnt(chIdx, 0);
    buckets_[index][min_counter].flowkey_ = flowkey;
    ch->resetCnt(minCHIdx, val);
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void CHElasticSketch<key_len, no_layer, T, hash_t>::lightpartInsert(
    const FlowKey<key_len> &flowkey, T val) {
  cm_.update(flowkey, val);
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void CHElasticSketch<key_len, no_layer, T, hash_t>::update(const FlowKey<key_len> &flowkey,
                                          T val) {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
T CHElasticSketch<key_len, no_layer, T, hash_t>::heavypartQuery(
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
T CHElasticSketch<key_len, no_layer, T, hash_t>::lightpartQuery(
    const FlowKey<key_len> &flowkey) const {
  return cm_.query(flowkey);
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
T CHElasticSketch<key_len, no_layer, T, hash_t>::query(const FlowKey<key_len> &flowkey) const {
