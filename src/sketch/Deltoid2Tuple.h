...
class Deltoid2Tuple : public SketchBase<key_len, T> {
private:
  T sum_;
  int32_t num_hash_;
  int32_t num_group_;
...
public:
   ...
   *
   */
  Deltoid2Tuple(int32_t num_hash, int32_t num_group);
  /**
...
Deltoid2Tuple<key_len, T, hash_t>::Deltoid2Tuple(int32_t num_hash, int32_t num_group)
...
template <int32_t key_len, typename T, typename hash_t>
Deltoid2Tuple<key_len, T, hash_t>::~Deltoid2Tuple() {
  if (arr1_ != nullptr) {
    delete[] arr1_[0][0];
    for (int32_t i = 0; i < num_hash_; ++i) {
      delete[] arr1_[i];
...
template <int32_t key_len, typename T, typename hash_t>
void Deltoid2Tuple<key_len, T, hash_t>::update(const FlowKey<key_len> &flowkey,
                                         T val) {
  sum_ += val;
  for (int32_t i = 0; i < num_hash_; ++i) {
...
template <int32_t key_len, typename T, typename hash_t>
T Deltoid2Tuple<key_len, T, hash_t>::query(const FlowKey<key_len> &flowkey) const {
...
Deltoid2Tuple<key_len, T, hash_t>::getHeavyHitter(double threshold) const {
  T thresh = threshold;
  double val1 = 0;
  double val0 = 0;
...
template <int32_t key_len, typename T, typename hash_t>
size_t Deltoid2Tuple<key_len, T, hash_t>::size() const {
  return sizeof(Deltoid2Tuple<key_len, T, hash_t>) +
         (2 * num_group_ * num_hash_ * nbits_ + 1) * sizeof(T) +
         num_hash_ * sizeof(hash_t);
}

template <int32_t key_len, typename T, typename hash_t>
void Deltoid2Tuple<key_len, T, hash_t>::clear() {
  sum_ = 0;
