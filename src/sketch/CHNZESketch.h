 ...
 * @file CHNZESketch.h
 * @author XierLabber
 * @brief Implementation of CHNZE Sketch
 *
...
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t>
class CHNZESketch : public SketchBase<key_len, T> {
...
public:
  ...
  CounterHierarchy<no_layer, T, hash_t> *FS;
...
public:
  CHNZESketch(int32_t HTLength, int32_t BFBitsNum, int32_t BFHashNum, 
      int32_t FSdepth, int32_t FSwidth, double cnt_no_ratio,
      const std::vector<size_t> &width_cnt,
      const std::vector<size_t> &no_hash);
  ~CHNZESketch();
  CHNZESketch(CHNZESketch &&) = delete;
  CHNZESketch &operator=(const CHNZESketch &) = delete;
  CHNZESketch &operator=(CHNZESketch &&) = delete;
...
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t>
CHNZESketch<key_len, no_layer, T, hash_t>::CHNZESketch(
    ...
    // CH
    FS = new CounterHierarchy<no_layer, T, hash_t>(no_cnt, this->width_cnt,
                                                    this->no_hash);
...
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t>
void CHNZESketch<key_len, no_layer, T, hash_t>::clear(){
...
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t>
void CHNZESketch<key_len, no_layer, T, hash_t>::update(const FlowKey<key_len> &flow_key, T val){
...
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t>
void CHNZESketch<key_len, no_layer, T, hash_t>::recover(){
...
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t>
T CHNZESketch<key_len, no_layer, T, hash_t>::query(const FlowKey<key_len> &flow_key) const{
  if(!have_decoded){
    const_cast<CHNZESketch<key_len, no_layer, T, hash_t>* >(this)->recover();
  }
  T ans = 0;
...
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t>
size_t CHNZESketch<key_len, no_layer, T, hash_t>::size() const{
    FS->print_rate("");
    return sizeof(*this)
