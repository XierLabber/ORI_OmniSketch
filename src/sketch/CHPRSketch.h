 ...
 * @tparam no_layer numbers of layers of CH
 * @tparam T        type of the counter
 * @tparam hash_t   hashing class
 */
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
class CHPRSketch : public SketchBase<key_len, T> {
private:
...
public:
   ...
   *
   */
  CHPRSketch(int32_t counter_length, int32_t counter_hash_num, 
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
CHPRSketch<key_len, no_layer, T, hash_t>::CHPRSketch(int32_t counter_length, 
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
CHPRSketch<key_len, no_layer, T, hash_t>::~CHPRSketch(){
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void CHPRSketch<key_len, no_layer, T, hash_t>::update(const FlowKey<key_len> &flowkey,
                                          T val) {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void CHPRSketch<key_len, no_layer, T, hash_t>::recover(){
  ...
  printf("\nDECODE COST %ldms FOR ALL\n", static_cast<int64_t>(MY_TIMER.count()));
#endif

}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
size_t CHPRSketch<key_len, no_layer, T, hash_t>::size() const{
    ch->print_rate("");
    return ch->size()
           + (filter_length >> 3)
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
T CHPRSketch<key_len, no_layer, T, hash_t>::query(const FlowKey<key_len> &flowkey) const{
    if(recovered_value.empty()){
        const_cast<CHPRSketch<key_len, no_layer, T, hash_t>* >(this)->recover();
    }
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void CHPRSketch<key_len, no_layer, T, hash_t>::clear(){
    ch->clear();
