...
class THD_CHFlowRadar : public SketchBase<key_len, T> {
private:
  ...
  THD_CHFlowRadar(const THD_CHFlowRadar &) = delete;
  THD_CHFlowRadar(THD_CHFlowRadar &&) = delete;
...
public:
   ...
   */
  THD_CHFlowRadar(int32_t flow_filter_size, int32_t flow_filter_hash,
...
THD_CHFlowRadar<key_len, no_layer, T, hash_t>::THD_CHFlowRadar(
    ...
    : num_bitmap(Util::NextPrime(flow_filter_size)),
      ...
      flow_width_cnt(flow_width_cnt), flow_no_cnt(flow_no_cnt), 
      packet_width_cnt(packet_width_cnt), packet_no_cnt(packet_no_cnt){
  // check ratio
  ...
  // CH
  flow_ch = new CounterHierarchy<no_layer, T, hash_t>(flow_no_cnt, 
                                                      flow_width_cnt,
                                                      flow_no_hash);
  packet_ch = new CounterHierarchy<no_layer, T, hash_t>(packet_no_cnt, 
                                                        packet_width_cnt,
                                                        packet_no_hash);
}
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void THD_CHFlowRadar<key_len, no_layer, T, hash_t>::update(const FlowKey<key_len> &flowkey,
                                           T val) {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
Data::Estimation<key_len, T> THD_CHFlowRadar<key_len, no_layer, T, hash_t>::decode() {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
size_t THD_CHFlowRadar<key_len, no_layer, T, hash_t>::size() const {
...
template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void THD_CHFlowRadar<key_len, no_layer, T, hash_t>::clear() {
  // reset flow counter
  num_flows = 0;
  // reset flow filter
  flow_filter->clear();
