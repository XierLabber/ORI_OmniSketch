...
#include <sketch/DHSketch.h>

#define DHS_PARA_PATH "DHS.para"
#define DHS_TEST_PATH "DHS.test"
#define DHS_DATA_PATH "DHS.data"

namespace OmniSketch::Test {

/**
 * @brief Testing class for DH Sketch
 *
 */
template <int32_t key_len, typename T, typename hash_t = Hash::AwareHash>
class DHSketchTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  DHSketchTest(const std::string_view config_file)
      : TestBase<key_len, T>("DHS", config_file, DHS_TEST_PATH) {}
...
void DHSketchTest<key_len, T, hash_t>::runTest() {
  /**
  ...
  ///
  int32_t bucketNum;  // sketch config
  ...
  parser.setWorkingNode(
      DHS_PARA_PATH); // do not forget to to enclose it with braces
  /// Step iv. Parse num_bits and num_hash
  if (!parser.parseConfig(bucketNum, "bucketNum"))
    return;
  /// Step v. Move to the data node
  parser.setWorkingNode(DHS_DATA_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::DHSketch<key_len, T, hash_t>(bucketNum));
