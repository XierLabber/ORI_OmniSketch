...
#include <sketch/THD_CHCMSketch.h>
...
class THD_CHCMSketchTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  THD_CHCMSketchTest(const std::string_view config_file)
...
void THD_CHCMSketchTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::THD_CHCMSketch<key_len, no_layer, T, hash_t>(
