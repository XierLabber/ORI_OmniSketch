/**
 * @file CHNZESketchTest.h
 * @author dromniscience (you@domain.com)
 * @brief Test CH-optimized NZE Sketch
 *
 * @copyright Copyright (c) 2022
 *
 */
#include <common/test.h>
#include <sketch/CHNZESketch.h>

#define CHNZE_PARA_PATH "NZE.para"
#define CHNZE_TEST_PATH "NZE.test"
#define CHNZE_DATA_PATH "NZE.data"
#define CHNZE_CH_PATH "NZE.ch"
...
class CHNZESketchTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  CHNZESketchTest(const std::string_view config_file)
      : TestBase<key_len, T>("NZE with CH", config_file, CHNZE_TEST_PATH) {
  }
...
void CHNZESketchTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  parser.setWorkingNode(
      CHNZE_PARA_PATH); // do not forget to to enclose it with braces
  ...
  parser.setWorkingNode(CHNZE_DATA_PATH);
  ...
  parser.setWorkingNode(CHNZE_CH_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::CHNZESketch<key_len, no_layer, T, hash_t>(
...
#undef CHNZE_PARA_PATH
#undef CHNZE_TEST_PATH
#undef CHNZE_DATA_PATH
#undef CHNZE_CH_PATH
