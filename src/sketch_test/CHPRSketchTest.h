/**
 * @file CHPRSketchTest.h
 * @author dromniscience (you@domain.com)
 * @brief Test CH-optimized PR Sketch Sketch
 *
 * @copyright Copyright (c) 2022
 *
 */
#include <common/test.h>
#include <sketch/CHPRSketch.h>

#define CHPR_PARA_PATH "PR.para"
#define CHPR_TEST_PATH "PR.test"
#define CHPR_DATA_PATH "PR.data"
#define CHPR_CH_PATH "PR.ch"
...
class CHPRSketchTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  CHPRSketchTest(const std::string_view config_file)
      : TestBase<key_len, T>("PR Sketch with CH", config_file, CHPR_TEST_PATH) {
  }
...
void CHPRSketchTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  parser.setWorkingNode(
      CHPR_PARA_PATH); // do not forget to to enclose it with braces
  ...
  parser.setWorkingNode(CHPR_DATA_PATH);
  ...
  parser.setWorkingNode(CHPR_CH_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::CHPRSketch<key_len, no_layer, T, hash_t>(
...
#undef CHPR_PARA_PATH
#undef CHPR_TEST_PATH
#undef CHPR_DATA_PATH
#undef CHPR_CH_PATH
