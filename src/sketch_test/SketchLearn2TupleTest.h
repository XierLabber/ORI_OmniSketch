...
#include <sketch/SketchLearn2Tuple.h>

#define SL2_PARA_PATH "SL2.para"
#define SL2_TEST_PATH "SL2.test"
#define SL2_DATA_PATH "SL2.data"
...
class SketchLearn2TupleTest : public TestBase<key_len, T> {
...
public:
   ...
   */
  SketchLearn2TupleTest(const std::string_view config_file)
      : TestBase<key_len, T>("Sketch Learn", config_file, SL2_TEST_PATH) {}
...
void SketchLearn2TupleTest<key_len, T, hash_t>::runTest() {
  /**
  ...
  parser.setWorkingNode(
      SL2_PARA_PATH); // do not forget to to enclose it with braces
  /// Step iv. Parse num_bits and num_hash
  if (!parser.parseConfig(depth, "depth"))
    return;
  if (!parser.parseConfig(width, "width"))
    return;
  /// Step v. To know about the data, we  switch to [SL2.data].
  parser.setWorkingNode(SL2_DATA_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::SketchLearn2Tuple<key_len, T, hash_t>(depth, width));
  ...
  if (hx_method == Data::TopK) {
    ...
    this->testHeavyHitter(
        ...
        gnd_truth_heavy_hitters); // gnd_truth_heavy_hitter: >, yet SketchLearn2Tuple: >=
  }
  ///        3. size
  this->testSize(ptr);
  ///        3. show metrics
