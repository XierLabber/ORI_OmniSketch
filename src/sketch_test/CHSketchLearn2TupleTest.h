/**
 * @file CHSketchLearn2TupleTest.h
 * @author XierLabber (lsmfttb@gmail.com)
 * @brief Test CHSketchLearn2Tuple
 *
...
#include <sketch/CHSketchLearn2Tuple.h>

#define CHSL2_PARA_PATH "SL2.para"
#define CHSL2_TEST_PATH "SL2.test"
#define CHSL2_DATA_PATH "SL2.data"
#define CHSL2_CH_PATH "SL2.ch"

namespace OmniSketch::Test {

/**
 * @brief Testing class for CHSketchLearn2Tuple
 *
 */
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t = Hash::AwareHash>
class CHSketchLearn2TupleTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  CHSketchLearn2TupleTest(const std::string_view config_file)
      : TestBase<key_len, T>("SketchLearn with CH", config_file, CHSL2_TEST_PATH) {}
...
void CHSketchLearn2TupleTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  /// Step iii. Set the working node of the parser. In this case, [CHSketchLearn2Tuple].
  parser.setWorkingNode(
      CHSL2_PARA_PATH); // do not forget to to enclose it with braces
  /// Step iv. Parse depth, width and threshold
  if (!parser.parseConfig(depth, "depth"))
    return;
  if (!parser.parseConfig(width, "width"))
    return;
  /// Step v. To know about the data, we move to the [CHSketchLearn2Tuple.data] node.
  parser.setWorkingNode(CHSL2_DATA_PATH);
  ...
  parser.setWorkingNode(CHSL2_CH_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::CHSketchLearn2Tuple<key_len, no_layer, T, hash_t>(depth, width, cnt_no_ratio, width_cnt, no_hash));
...
#undef CHSL2_PARA_PATH
#undef CHSL2_TEST_PATH
#undef CHSL2_DATA_PATH
#undef CHSL2_CH_PATH
