...
#include <sketch/THD_CHHHUnivMon.h>
...
class THD_CHHHUnivMonTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  THD_CHHHUnivMonTest(const std::string_view config_file)
...
void THD_CHHHUnivMonTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::THD_CHHHUnivMon<key_len, no_layer, T, hash_t>(
