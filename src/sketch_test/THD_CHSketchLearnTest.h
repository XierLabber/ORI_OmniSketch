/**
 * @file THD_CHSketchLearnTest.h
 * @author XierLabber (lsmfttb@gmail.com)
 * @brief Test THD_CHSketchLearn
 *
...
#include <common/test_thd.h>
#include <sketch/THD_CHSketchLearn.h>

#define CHSL_PARA_PATH "SL.para"
#define CHSL_TEST_PATH "SL.test"
#define CHSL_DATA_PATH "SL.data"
#define CHSL_CH_PATH "SL.ch"

namespace OmniSketch::Test {

/**
 * @brief Testing class for THD_CHSketchLearn
 *
 */
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t = Hash::AwareHash>
class THD_CHSketchLearnTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  THD_CHSketchLearnTest(const std::string_view config_file)
      : TestBase<key_len, T>("SketchLearn with CH", config_file, CHSL_TEST_PATH) {}
...
void THD_CHSketchLearnTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  /// Step iii. Set the working node of the parser. In this case, [THD_CHSketchLearn].
  parser.setWorkingNode(
      CHSL_PARA_PATH); // do not forget to to enclose it with braces
  /// Step iv. Parse depth, width and threshold
  if (!parser.parseConfig(depth, "depth"))
    return;
  if (!parser.parseConfig(width, "width"))
    return;
  /// Step v. To know about the data, we move to the [THD_CHSketchLearn.data] node.
  parser.setWorkingNode(CHSL_DATA_PATH);
  ...
  parser.setWorkingNode(CHSL_CH_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::THD_CHSketchLearn<key_len, no_layer, T, hash_t>(depth, width, cnt_no_ratio, width_cnt, no_hash));
...
#undef CHSL_PARA_PATH
#undef CHSL_TEST_PATH
#undef CHSL_DATA_PATH
#undef CHSL_CH_PATH
