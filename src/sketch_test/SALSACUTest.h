...
#include <sketch/SALSACU.h>

#define SSCU_PARA_PATH "SSCU.para"
#define SSCU_TEST_PATH "SSCU.test"
#define SSCU_DATA_PATH "SSCU.data"

namespace OmniSketch::Test {

/**
 * @brief Testing class for SALSA CU Sketch
 *
 */
template <int32_t key_len, typename T, typename hash_t = Hash::AwareHash>
class SALSACUTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  SALSACUTest(const std::string_view config_file)
      : TestBase<key_len, T>("SALSA CU", config_file, SSCU_TEST_PATH) {}
...
void SALSACUTest<key_len, T, hash_t>::runTest() {
  /**
  ...
  parser.setWorkingNode(
      SSCU_PARA_PATH); // do not forget to to enclose it with braces
  ...
  parser.setWorkingNode(SSCU_DATA_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::SALSACU<key_len, T, hash_t>(depth, width));
...
#undef SSCU_PARA_PATH
#undef SSCU_TEST_PATH
#undef SSCU_DATA_PATH
