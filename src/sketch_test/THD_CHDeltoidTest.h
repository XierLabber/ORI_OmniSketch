/**
 * @file THD_CHDeltoidTest.h
 * @author XierLabber (lsmfttb@gmail.com)
 * @brief Test THD_CHDeltoid
 *
...
#include <sketch/THD_CHDeltoid.h>

#define CHDT_PARA_PATH "DT.para"
#define CHDT_TEST_PATH "DT.test"
#define CHDT_DATA_PATH "DT.data"
#define CHDT_CH_PATH "DT.ch"

namespace OmniSketch::Test {

/**
 * @brief Testing class for THD_CHDeltoid
 *
 */
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t = Hash::AwareHash>
class THD_CHDeltoidTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  THD_CHDeltoidTest(const std::string_view config_file)
      : TestBase<key_len, T>("Deltoid with CH", config_file, CHDT_TEST_PATH) {}
...
void THD_CHDeltoidTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  /// Step iii. Set the working node of the parser. In this case, [THD_CHDeltoid].
  parser.setWorkingNode(
      CHDT_PARA_PATH); // do not forget to to enclose it with braces
  ...
  if (!parser.parseConfig(num_group, "num_group"))
    return;
  /// Step v. To know about the data, we move to the [THD_CHDeltoid.data] node.
  parser.setWorkingNode(CHDT_DATA_PATH);
  ...
  parser.setWorkingNode(CHDT_CH_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::THD_CHDeltoid<key_len, no_layer, T, hash_t>(num_hash, num_group, cnt_no_ratio, width_cnt, no_hash));
...
#undef CHDT_PARA_PATH
#undef CHDT_TEST_PATH
#undef CHDT_DATA_PATH
#undef CHDT_CH_PATH
