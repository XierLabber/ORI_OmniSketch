/**
 * @file CHNitroSketch.h
 * @author XierLabber
 * @brief Test CHNitroSketch
 *
...
#include <sketch/CHNitroSketch.h>
...
/**
 * @brief Testing class for CHNitroSketch
 *
 */
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t = Hash::AwareHash>
class CHNitroSketchTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  CHNitroSketchTest(const std::string_view config_file)
...
void CHNitroSketchTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::CHNitroSketch<key_len, no_layer, T, hash_t>(
