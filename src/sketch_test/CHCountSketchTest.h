...
#include <sketch/CHCountSketch.h>
...
class CHCountSketchTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  CHCountSketchTest(const std::string_view config_file)
...
void CHCountSketchTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::CHCountSketch<key_len, no_layer, T, hash_t>(
