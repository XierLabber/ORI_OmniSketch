/**
 * @file THD_CHNitroSketch.h
 * @author XierLabber
 * @brief Test THD_CHNitroSketch
 *
...
#include <sketch/THD_CHNitroSketch.h>

#define CHNS_PARA_PATH "NS.para"
#define CHNS_TEST_PATH "NS.test"
#define CHNS_DATA_PATH "NS.data"
#define CHNS_CH_PATH "NS.ch"

namespace OmniSketch::Test {

/**
 * @brief Testing class for THD_CHNitroSketch
 *
 */
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t = Hash::AwareHash>
class THD_CHNitroSketchTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  THD_CHNitroSketchTest(const std::string_view config_file)
      : TestBase<key_len, T>("Nitro Sketch with CH", config_file, CHNS_TEST_PATH) {}
...
void THD_CHNitroSketchTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  parser.setWorkingNode(
      CHNS_PARA_PATH); // do not forget to to enclose it with braces
  ...
  parser.setWorkingNode(CHNS_DATA_PATH);
  ...
  parser.setWorkingNode(CHNS_CH_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::THD_CHNitroSketch<key_len, no_layer, T, hash_t>(
...
#undef CHNS_PARA_PATH
#undef CHNS_TEST_PATH
#undef CHNS_DATA_PATH
