...
#include <sketch/THD_CHFlowRadar.h>
...
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t>
class THD_CHFlowRadarTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  THD_CHFlowRadarTest(const std::string_view config_file)
...
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t>
void THD_CHFlowRadarTest<key_len, no_layer, T, hash_t>::runTest(){
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::THD_CHFlowRadar<key_len, no_layer, T, hash_t>(
