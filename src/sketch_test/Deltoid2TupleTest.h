/**
 * @file Deltoid2TupleTest.h
 * @author deadlycat (lsmfttb@gmail.com)
 * @brief Test Deltoid2Tuple
 *
...
#include <sketch/Deltoid2Tuple.h>

#define DT2_PARA_PATH "DT2.para"
#define DT2_TEST_PATH "DT2.test"
#define DT2_DATA_PATH "DT2.data"

namespace OmniSketch::Test {

/**
 * @brief Testing class for Deltoid2Tuple
 *
 */
template <int32_t key_len, typename T, typename hash_t = Hash::AwareHash>
class Deltoid2TupleTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  Deltoid2TupleTest(const std::string_view config_file)
      : TestBase<key_len, T>("Deltoid", config_file, DT2_TEST_PATH) {}
...
void Deltoid2TupleTest<key_len, T, hash_t>::runTest() {
  /**
  ...
  /// Step iii. Set the working node of the parser. In this case, [Deltoid2Tuple].
  parser.setWorkingNode(
      DT2_PARA_PATH); // do not forget to to enclose it with braces
  ...
  if (!parser.parseConfig(num_group, "num_group"))
    return;
  /// Step v. To know about the data, we move to the [Deltoid2Tuple.data] node.
  parser.setWorkingNode(DT2_DATA_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::Deltoid2Tuple<key_len, T, hash_t>(num_hash, num_group));
...
//    TEMPLATE: <8, int32_t, Hash::AwareHash>