/**
 * @file CHDeltoidTest.h
 * @author XierLabber (lsmfttb@gmail.com)
 * @brief Test CHDeltoid
 *
...
public:
   ...
   * '.')
   */
  CHDeltoidTest(const std::string_view config_file)
...
void CHDeltoidTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  /// Step iii. Set the working node of the parser. In this case, [CHDeltoid].
  ...
  if (!parser.parseConfig(num_group, "num_group"))
    return;
  /// Step v. To know about the data, we move to the [CHDeltoid.data] node.
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::CHDeltoid<key_len, no_layer, T, hash_t>(num_hash, num_group, cnt_no_ratio, width_cnt, no_hash));
