 ...
 * @brief Test SALSA Count Min Sketch
 *
...
#include <sketch/SALSACM.h>

#define SSCM_PARA_PATH "SSCM.para"
#define SSCM_TEST_PATH "SSCM.test"
#define SSCM_DATA_PATH "SSCM.data"

namespace OmniSketch::Test {

/**
 * @brief Testing class for SALSA Count Min Sketch
 *
 */
template <int32_t key_len, typename T, typename hash_t = Hash::AwareHash>
class SALSACMTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  SALSACMTest(const std::string_view config_file)
      : TestBase<key_len, T>("SALSA CM", config_file, SSCM_TEST_PATH) {}
...
void SALSACMTest<key_len, T, hash_t>::runTest() {
  /**
  ...
  parser.setWorkingNode(
      SSCM_PARA_PATH); // do not forget to to enclose it with braces
  ...
  parser.setWorkingNode(SSCM_DATA_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::SALSACM<key_len, T, hash_t>(depth, width));
...
#undef SSCM_PARA_PATH
#undef SSCM_TEST_PATH
#undef SSCM_DATA_PATH
