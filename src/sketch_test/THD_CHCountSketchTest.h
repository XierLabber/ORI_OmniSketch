...
#include <sketch/THD_CHCountSketch.h>

#define CHCS_PARA_PATH "CS.para"
#define CHCS_TEST_PATH "CS.test"
#define CHCS_DATA_PATH "CS.data"
#define CHCS_CH_PATH "CS.ch"

namespace OmniSketch::Test {

/**
 * @brief Testing class for CH Count Sketch
 *
 */
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t = Hash::AwareHash>
class THD_CHCountSketchTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  THD_CHCountSketchTest(const std::string_view config_file)
      : TestBase<key_len, T>("Count Sketch with CH", config_file, CHCS_TEST_PATH) {}
...
void THD_CHCountSketchTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  parser.setWorkingNode(
      CHCS_PARA_PATH); // do not forget to to enclose it with braces
  ...
  parser.setWorkingNode(CHCS_DATA_PATH);
  ...
  parser.setWorkingNode(CHCS_CH_PATH);
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::THD_CHCountSketch<key_len, no_layer, T, hash_t>(
...
#undef CHCS_PARA_PATH
#undef CHCS_TEST_PATH
#undef CHCS_DATA_PATH
