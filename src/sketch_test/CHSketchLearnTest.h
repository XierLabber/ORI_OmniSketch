/**
 * @file CHSketchLearnTest.h
 * @author XierLabber (lsmfttb@gmail.com)
 * @brief Test CHSketchLearn
 *
...
#include <sketch/CHSketchLearn.h>
...
/**
 * @brief Testing class for CHSketchLearn
 *
 */
template <int32_t key_len, int32_t no_layer, typename T,
          typename hash_t = Hash::AwareHash>
class CHSketchLearnTest : public TestBase<key_len, T> {
...
public:
   ...
   * '.')
   */
  CHSketchLearnTest(const std::string_view config_file)
...
void CHSketchLearnTest<key_len, no_layer, T, hash_t>::runTest() {
  /**
  ...
  /// Step iii. Set the working node of the parser. In this case, [CHSketchLearn].
  ...
  if (!parser.parseConfig(width, "width"))
    return;
  /// Step v. To know about the data, we move to the [CHSketchLearn.data] node.
  ...
  std::unique_ptr<Sketch::SketchBase<key_len, T>> ptr(
      new Sketch::CHSketchLearn<key_len, no_layer, T, hash_t>(depth, width, cnt_no_ratio, width_cnt, no_hash));
